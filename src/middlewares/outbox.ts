import { Create, Follow, Note, Recipient, signRequest } from '@fedify/fedify'
...
export const processActivity: Middleware<{
  ...
  switch (validActivity.type) {
    ...
    case 'Note': {
      ...
      const to: Parameters<typeof fedifyContext.sendActivity>[1] =
        isForFollowers ? 'followers' : ([] as Recipient[])

      // send the activity out
      await fedifyContext.sendActivity(
        { identifier: ctx.state.owner.actor.id },
        to,
        new Create({
